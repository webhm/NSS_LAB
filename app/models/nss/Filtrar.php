...
class Filtrar extends Models implements IModels
{
    ...
    public function filtrarOrdenes()
    {
        ...
        try {
            ...
            foreach ($list as $key => $val) {
                ...
                if (isset($documento['fechaExamen']) && $documento['fechaExamen'] !== date('Y-m-d')) {

                    if ($i <= 5 && $documento['statusFiltro'] == 0) {

                        $dataOrderPCR = $this->extraerDataPCR($documento);
                        $dataOrder = $this->extraerData($documento);

                        # PARA PCR:
                        if ($dataOrderPCR['tipo'] == 'PCR' && count($dataOrderPCR['data']) !== 0) {
                            $documento['_PCR'] = 1;
                            $documento['orderTests'] = $dataOrderPCR['data'];
                        }

                        # PARA RUTINA
                        if ($dataOrderPCR['tipo'] == 'RUTINA' && $dataOrder['tipo'] == 'RUTINA' && count($dataOrder['data']) !== 0) {
                            $documento['orderTests'] = $dataOrder['data'];
                        }
    ...
    public function verificarPCR($orderTests)
    {

        $esPCR = false;

        foreach ($orderTests as $key => $value) {
    ...
    public function filtrarOrdenesDia()
    {
    ...
    public function filtrarReglas()
    {
        ...
        foreach ($this->ordenes as $key) {
            ...
            // Filtrada Para Envío
            if (count($this->documento['orderTests']) !== 0 && $this->documento['sc'] == $key['sc']) {

                if (count($this->documento['reglasFiltrosEnvio']) !== 0 && count($this->documento['reglasFiltrosNoEnvio']) == 0) {
    ...
    public function filtrarPor($tipo, $proceso)
    {
        ...
        # Validacion personalizadas de Preubas
        if ($tipo == 'pruebas' && $proceso == 1) {
            ...
            foreach ($this->reglasEnvio as $key) {
                ...
                if (!is_null($key['xIdPrueba'])) {
                    ...
                    if (count($this->documento['orderTests']) !== 0) {
    ...
    public function extraerDataPCR($documento)
    {
        ...
        try {
            ...
            if (!isset($array['DefaultDataSet']['SQL']['rTests'])) {
                ...
                $test = array(
                    'TestID' => $array['DefaultDataSet']['SQL']['rTests'],
                );
            ...
            if (count($tests) !== 0) {
                ...
                foreach ($tests as $key) {
                    ...
                    if ($key['TestID'] == '9720' || $key['TestID'] == '9719') {
                        $esPCR = true;
                    }
    ...
    public function extraerData($documento)
    {

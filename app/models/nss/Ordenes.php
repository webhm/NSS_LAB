...
class Ordenes extends Models implements IModels
{
    ...
    public function extraerOrdenesDirefirido(): array
    {
        ...
        try {
            ...
            $time = new DateTime();
            $nuevaHora = strtotime('-1 days', $time->getTimestamp());
            $fechaConsulta = date('Y-m-d', $nuevaHora);
            ...
            $Preview = $client->GetList(
                array(
                    ...
                    'pstrRegisterDateFrom' => $fechaConsulta,
                    'pstrRegisterDateTo' => $fechaConsulta,
                    'pstrUse' => 5,
                )
            );
            ...
            foreach ((array) $array['DefaultDataSet']['SQL'] as $key) {
                ...
                if (isset($key['SampleID']) && isset($key['RegisterDate'])) {
    ...
    public function crearOrden($documento)
    {
        ...
        if (@file_get_contents($stsLogPen, true) === false) {
            ...
            file_put_contents('ordenes/logs/sc_' . $documento['sc'] . '_' . $documento['fechaExamen'] . '.json', json_encode($documento), FILE_APPEND);
            file_put_contents('ordenes/ingresadas/sc_' . $documento['sc'] . '_' . $documento['fechaExamen'] . '.json', json_encode($documento), FILE_APPEND);

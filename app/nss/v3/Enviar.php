...
class Enviar extends Models implements IModels
{
        ...
        try {
            ...
            foreach ($this->ordenes as $key) {
                ...
                if ($this->documento['statusEnvio'] == 4 && $this->documento['sc'] == $key['sc']) {
                    ...
                    $this->documento['numEnvios'] = $this->documento['enviosParciales'];

                    foreach ($this->documento['reglasFiltrosEnvio'] as $k => $v) {
                        if ($v['statusEnvio'] == 0) {
                            $this->documento['reglasFiltrosEnvio'][$k]['statusEnvio'] = 1;
                        }
                    }
    ...
    public function enviarResultado()
    {
        ...
        foreach ($documento['reglasFiltrosEnvio'] as $key => $val) {
            ...
            # Para validaciones totales
            if ($val['tipoValidacion'] == 1 && $documento['tipoValidacion'] == 1 && $documento['validacionClinica'] == 1 && $documento['validacionMicro'] == 1) {
                ...
                if ($direcciones['dirrecciones'] == '1') {
                    ...
                    if ($stsEnvio['status']) {
                        $documento['logsEnvio'][] = $stsEnvio['data'];
                        $envioExitoso = true;
                    }
                }
            ...
            if ($val['tipoValidacion'] == 1 && $documento['tipoValidacion'] == 1 && $documento['validacionClinica'] == 1 && $documento['validacionMicro'] == 0) {
            ...
            if ($val['tipoValidacion'] == 1 && $documento['tipoValidacion'] == 1 && $documento['validacionClinica'] == 0 && $documento['validacionMicro'] == 1) {
        ...
        # Validaciones para generar envios
        if (count($documento['validacionesParciales']) !== 0 || $documento['validacionMicro'] !== 0 || $documento['validacionClinica'] !== 0) {
            $necesitaEnvio = true;
        }
        ...
        if ($necesitaEnvio) {
            ...
            if ($stsEnvio['status']) {
                ...
                $documento['logsEnvio'][] = array('correos' => $stsEnvio['data'][0]);
                $envioExitoso = true;
                foreach ($documento['reglasFiltrosEnvio'] as $k => $v) {
                    if (in_array($v['xIdPrueba'], $documento['validacionesParciales'])) {
                        $documento['reglasFiltrosEnvio'][$k]['statusEnvio'] = 1;
                    }
                }
        ...
        if ($envioExitoso) {
            ...
            # VALIDAR STATUS RESULTADOS

            if (!$this->verificarStatus($documento)) {
